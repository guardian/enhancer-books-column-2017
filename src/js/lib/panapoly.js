var panapoly = {
    ...
    initialize: function() {
        ...
        if (panapoly.listening == null) {
            ...
            // Now we know if the device is a mobile with a gyroscope or a normall browser
    ...
    createPano: function() {
        ...
        document.querySelectorAll('.panorama').forEach(function(panorama, index) {

           if (!panorama.hasAttribute("id")) {

               panorama.id = "panorama_" + index;
    ...
    gyroSwitch: function() {
        ...
        var isAndroidApp = (/(android)/i.test(navigator.userAgent)) ? true : false; //window.location.origin === "file://" && 

        (isAndroidApp) ? panapoly.panoIframe() : panapoly.createPano();

    },

    panoIframe: function() {

        document.querySelectorAll('.panorama_block').forEach(function(panorama, index) {

           if (!panorama.hasAttribute("id")) {

                var para = {
                    "a": "https://interactive.guim.co.uk/embed/2017/07/360/R0010100.jpg",
                    "b": 1,
                    "c": -135.4,
                    "d": 120
                };

                panorama.id = "panorama_block_" + index;

                (panapoly.getPano(index)) ? para = panapoly.getPano(index) : '';

                var iframe = '<iframe src="https://interactive.guim.co.uk/testing/2017/07/360-photo-viewer/embed/embed.html?a=' + para.a +'" width="100%" height="300"></iframe>';

                var target = document.getElementById('panorama_block_' + index);
